...
IF EXISTS (SELECT name FROM sys.databases WHERE name = N'Qly_HTBanHangDoAnThucUong')
BEGIN
    ALTER DATABASE Qly_HTBanHangDoAnThucUong SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
...
CREATE TABLE ChuongTrinhKhuyenMai(
   ...
   MaGianHang INT NULL,
   CONSTRAINT GioiHanNgay CHECK (NgayKetThuc > NgayBatDau),
...
BEGIN
    ...
    VALUES (1, 1, @orderDate, 150000 + (@i * 10000), 0, 150000 + (@i * 10000), N'Hoàn thành');
    SET @newOrderId = SCOPE_IDENTITY();
    INSERT INTO CT_DonHang (MaDonHang, MaBienThe, SoLuongMua, GiaBan) VALUES (@newOrderId, 1, 1, 150000 + (@i * 10000));
    
    -- Gian Hàng 2
    INSERT INTO DonHang (MaKH, MaGianHang, NgayTaoDon, TongTienMon, GiamGia, ThanhTienKhachTra, ThanhTienQuanNhan, TrangThaiDonHang)
    VALUES (2, 2, @orderDate, 200000 - (@i * 5000), 0, 200000 - (@i * 5000), N'Hoàn thành');
    SET @newOrderId = SCOPE_IDENTITY();
...
========================= */
INSERT INTO ChuongTrinhKhuyenMai (TenChTrinh, NoiDungKMai, NgayBatDau, NgayKetThuc, PhanTramGiam, GiamToiDa, DieuKienApDung, SuDung1Lan, LoaiKhuyenMai, TrangThaiKMai, MaGianHang) VALUES
