...
for i in range(0, len(lines), 4):
    ...
    old_ward = lines[i+2]
    new_ward = lines[i+3]
    records.append((stt, city, old_ward, new_ward))

insert_statements = []
insert_statements.append('\n/* =========================\n   DỮ LIỆU 168 PHƯỜNG/XÃ MỚI SÁP NHẬP\n========================= */\n')
insert_statements.append('INSERT INTO DiaChi (MaKh, MaGianHang, TenGoiNho, TenNguoiNhan, SoDienThoaiNhan, DiaChiCuThe, PhuongXa, ThanhPho, ViDo, KinhDo, LaMacDinh, DaXoa) VALUES\n')

values = []
for idx, r in enumerate(records):
    city = r[1].replace("'", "''")
    new_ward = r[3].replace("'", "''")
    makh = (idx % 10) + 1
    val = f"({makh}, NULL, N'Địa chỉ sáp nhập', N'Khách Hàng {makh}', '0900000{str(idx).zfill(3)}', N'Nhà số {idx+1}', N'{new_ward}', N'{city}', 10.7, 106.6, 0, 0)"
    values.append(val)

insert_sql = insert_statements[0] + insert_statements[1] + ',\n'.join(values) + ';\nGO\n'

file_path = r'd:\Khóa Luận Cử Nhân\HeThongBanHang\QuanLyBanHangDoAnThucUong (8)\QuanLyBanHangDoAnThucUong\resources\SQLQuery.sql'
with codecs.open(file_path, 'a', 'utf-8-sig') as f:
    f.write(insert_sql)
